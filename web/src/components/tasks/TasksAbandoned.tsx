...
const TasksAbandoned: React.FC<TasksAbandonedProps> = ({ tasks, renderTask, allowSorting = false }) => {
  ...
  const handleShowLess = () => {
    setCurrentPage(1);
    // 滚动到已放弃任务区域的顶部
    const element = document.querySelector('[data-abandoned-tasks-section]');
    if (element) {
  ...
  return (
    <div className="border-t mt-4" data-abandoned-tasks-section>
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <div className="w-full p-3 text-sm text-gray-500">
          <CollapsibleTrigger className="w-full flex items-center justify-between">
            <span>已放弃 {tasks.length}</span>
            {isOpen ? (
        ...
        <CollapsibleContent>
          <Droppable droppableId="abandoned-tasks" isDropDisabled={!allowSorting}>
            {(provided, snapshot) => (
              <div
